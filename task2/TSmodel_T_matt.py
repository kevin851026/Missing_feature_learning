...
class Model(nn.Module):
    def __init__(self, input_size,hidden_size,category_size):
        ...
        self.n_head = 8
        self.w_qs = nn.Linear(1, self.n_head*128)
        self.w_ks = nn.Linear(1, self.n_head*128)
        ...
        self.classifier =  nn.Sequential(
                        nn.Linear(self.n_head*(input_size+7)+input_size,hidden_size*2),
                        nn.ReLU(),
                        nn.Dropout(drop_pro),
    ...
    def forward(self, features):
        ...
        new_features = torch.cat((features.clone(), regression1,regression2,regression3,regression4,regression5,regression6,regression7),dim=1)

        q = self.w_qs(new_features.view(batch_size,-1,1)).view(batch_size,14+7,self.n_head,-1)
        k = self.w_ks(new_features.view(batch_size,-1,1)).view(batch_size,14+7,self.n_head,-1)
        v = self.w_vs(new_features.view(batch_size,-1,1)).view(batch_size,14+7,self.n_head,-1)
        q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)
        attn = torch.matmul(q/np.power(14+7,0.5),k.transpose(2,3))
        attn = F.softmax(attn,-1)
        attn_feats = torch.matmul(attn,v)
        attn_feats = torch.cat((features,attn_feats.view(features.shape[0],-1)),dim=1)
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load('task2_matt.pkl'))
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task2_matt'+'.pkl')
        ...
        draw_chart(chart_data,'task2_matt')