...
class Model(nn.Module):
    def __init__(self, input_size,hidden_size,category_size):
        ...
        self.fc =  nn.Sequential(
                        # nn.Linear(self.channel*(input_size-self.kernel_size+1),hidden_size*1),
                        nn.Linear(14,hidden_size*1),
                        nn.SELU(),
                        nn.Dropout(drop_pro),
    ...
    def forward(self, features):
        ...
        conv1 = self.conv1(features.view(batch_size,1,-1))
        logits = self.fc(conv1.view(batch_size,-1))
        # new_feature = torch.cat((features,conv1.view(batch_size,-1)),dim=1)
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load('task2_moreF0.pkl'))
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task2_moreF0'+'.pkl')
        ...
        draw_chart(chart_data,'task2_moreF0')