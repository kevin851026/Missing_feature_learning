...
class Model(nn.Module):
    ...
    def forward(self, features):
        logits = self.fc2(self.fc1(features))
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load('task2_moreF0_3.pkl'))
    model = model.to(device)
    optimizer = optim.Adam(model.parameters(), lr=learning_rate,weight_decay=1e-5)
    max_acc = get_acc(dataset,model)
    print(max_acc)
    # get_upload(model)
    # exit()
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task2_moreF0_3'+'.pkl')
        ...
        draw_chart(chart_data,'task2_moreF0_3')