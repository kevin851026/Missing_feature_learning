...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('task2_base2.pkl'))
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task2_base2'+'.pkl')
        ...
        print('Epoch: ' , str(epoch) , \
              ...
              ' ',round(model.weight[0].item()/weight_sum,4) , \
              ' ',round((model.weight[1].item())/weight_sum,4))
        chart_data['epoch'].append(epoch)
        chart_data['tarin_loss'].append(train_ls_1/(step+1))
        chart_data['val_acc'].append(v_acc)
        draw_chart(chart_data,'task2_base2')