...
class Model(nn.Module):
    def __init__(self, input_size,hidden_size,category_size):
        ...
        self.classifier =  nn.Sequential(
                        nn.Linear(self.n_head*input_size,hidden_size*1),
                        nn.ReLU(),
                        nn.Dropout(drop_pro),
    ...
    def forward(self, features):
        ...
        q = self.w_qs(features.view(batch_size,-1,1)).view(batch_size,14,self.n_head,-1)
        k = self.w_ks(features.view(batch_size,-1,1)).view(batch_size,14,self.n_head,-1)
        v = self.w_vs(features.view(batch_size,-1,1)).view(batch_size,14,self.n_head,-1)
        q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)
        attn = torch.matmul(q,k.transpose(2,3))
        attn = F.softmax(attn,-1)
        feats = torch.matmul(attn,v)
        # new_features = torch.cat((features,feats.view(features.shape[0],-1)),dim=1)
        logits = self.classifier(feats.view(features.shape[0],-1))
        return logits
     # 1
     # 2
     # 3
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load('task2_attn_2.pkl'))
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task2_attn_2'+'.pkl')
        ...
        draw_chart(chart_data,'task2_attn_2')