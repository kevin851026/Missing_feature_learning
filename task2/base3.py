...
class Model(nn.Module):
    ...
    def forward(self, features):
        regression1 = self.fc1(features)
        regression1 = self.fc2(regression1)
        regression1 = self.regression1(regression1)

        regression2 = self.fc3(features)
        regression2 = self.fc4(regression2)
        regression2 = self.regression2(regression2)

        regression3 = self.fc5(features)
        regression3 = self.fc6(regression3)
        regression3 = self.regression3(regression3)

        new_features = features.clone().detach()
        new_features.T[1] = regression1.view(-1)
        new_features.T[6] = regression2.view(-1)
        new_features.T[11] = regression3.view(-1)
        logits = self.fc7(new_features)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('task2_base3.pkl'))
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task2_base3'+'.pkl')
        ...
        draw_chart(chart_data,'task2_base3')