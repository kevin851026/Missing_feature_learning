...
class Model(nn.Module):
    def __init__(self, input_size,hidden_size,category_size):
        ...
        self.fc6 =  nn.Sequential(
                        nn.Linear(input_size+5,hidden_size*2),
                        nn.SELU(),
                        nn.Dropout(drop_pro)
                    )
    ...
    def forward(self, features):
        ...
        regression4 = self.regression4(logits)
        logits = self.fc5(features)
        regression5 = self.regression5(logits)
        new_features = features.clone()
        new_features = torch.cat((new_features, regression1,regression2,regression3,regression4,regression5),dim=1)
        logits = self.fc6(new_features)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('task3_moreF6.pkl'))
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task3_moreF6'+'.pkl')
        ...
        draw_chart(chart_data,'task3_moreF6')