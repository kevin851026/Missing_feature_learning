...
def get_acc(dataset,model):
    ...
    for step, (batch) in enumerate(loader):
        input_tensor,miss_tensor,all_tensor,target_tensor = [t.to(device) for t in batch]
        logits,regression,generate = model(input_tensor)
        topv, topi = logits.topk(1)
...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            miss_tensor = torch.tensor([self.data.iloc[index,1]])
...
class Model(nn.Module):
    def __init__(self, input_size,hidden_size,category_size):
        ...
        self.n_head = 4
        self.w_qs = nn.Linear(1, self.n_head*4)
        self.w_ks = nn.Linear(1, self.n_head*4)
        self.w_vs = nn.Linear(1, self.n_head*1)
        self.classifier =  nn.Sequential(
                        nn.Linear((self.n_head+1)*(input_size+1+7),hidden_size*2),
                        nn.SELU(),
                        nn.Dropout(drop_pro),
                        nn.Linear(hidden_size*2,hidden_size*2),
                        nn.SELU(),
    ...
    def forward(self, features):
        ...
        new_features = features.clone()
        new_features = torch.cat((new_features, regression1),dim=1)
        ...
        expand_features = torch.cat((new_features, generate1,generate2,generate3,generate4,generate5,generate6,generate7),dim=1)
        batch_size = features.shape[0]
        q = self.w_qs(expand_features.view(batch_size,-1,1)).view(batch_size,8,self.n_head,-1)
        k = self.w_ks(expand_features.view(batch_size,-1,1)).view(batch_size,8,self.n_head,-1)
        v = self.w_vs(expand_features.view(batch_size,-1,1)).view(batch_size,8,self.n_head,-1)
        q, k, v = q.transpose(1, 2), k.transpose(1, 2), v.transpose(1, 2)
        attn = torch.matmul(q,k.transpose(2,3))
        attn = F.softmax(attn,-1)
        attn = torch.matmul(attn,v)
        logits = self.classifier(torch.cat((expand_features,attn.view(batch_size,-1)),dim=1))
        return logits,regression1,torch.cat((generate1,generate2,generate3,generate4,generate5,generate6,generate7),dim=1)
...
if __name__ == '__main__':
    ...
    for epoch in range(15000):
        ...
        for step, (batch) in enumerate(train_loader):
            ...
            loss4 = criterion3(logits, F.softmax(t_sample/24,dim=1))

            optimizer.zero_grad()
            loss = 0.9*loss1 + 0.5*loss2 + 0.5*loss3 + 0.1*loss4
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task3_student_4'+'.pkl')
        ...
        draw_chart(chart_data,'task3_student_4')