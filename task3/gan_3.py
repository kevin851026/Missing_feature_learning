...
def get_upload(model):
    ...
    for step, (batch) in enumerate(test_loader):
        ...
        regression = model.G(features = input_tensor)
        new_feature = torch.cat((input_tensor,regression),dim=1)
        logits = model.C(features = new_feature)
...
def get_acc(dataset,model):
    ...
    for step, (batch) in enumerate(loader):
        input_tensor,miss_tensor,c_input_tensor,target_tensor=[t.to(device) for t in batch]
        regression = model.G(features = input_tensor)
        new_feature = torch.cat((input_tensor,regression),dim=1)
        logits = model.C(features = new_feature)
        topv, topi = logits.topk(1)
...
class Dataset(Dataset):
    ...
    def __getitem__(self, index):
        if self.train:
            ...
            classifier_input = self.data.iloc[index,1:-1]
            classifier_input.drop(['F1','F5','F8'], inplace=True)
            classifier_input = classifier_input.values.astype(np.float32)
            ...
            c_input_tensor = torch.tensor(classifier_input)
            mask = torch.Tensor([1]*len(feature))
            # if self.val:
            #     for i in range(random.randint(0,3)):
            #         mask[random.randint(0,len(mask)-1)] = 0
            feature_tensor = feature_tensor*mask
            return feature_tensor,miss_tensor,c_input_tensor,label_tensor
        else:
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load('task3_gan_3.pkl'))
    model.D.load_state_dict(torch.load('task3_ganD.pkl'))
    model.G.load_state_dict(torch.load('task3_ganG.pkl'))
    ...
    for epoch in range(15000):
        ...
        for step, (batch) in enumerate(train_loader):
            input_tensor,miss_tensor,c_input_tensor,target_tensor = [t.to(device) for t in batch]
            d_real_logit = model.D(features = miss_tensor.view(-1,1))
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task3_gan_3'+'.pkl')
        print('Epoch: '+str(epoch)+\
        ...
        draw_chart(chart_data,'task3_gan_3')