...
class Model(nn.Module):
    def __init__(self, input_size,hidden_size,category_size):
        ...
        self.fc10 =  nn.Sequential(
                        nn.Linear(input_size+9,hidden_size*2),
                        nn.SELU(),
                        nn.Dropout(drop_pro)
                    )
    ...
    def forward(self, features):
        ...
        regression2 = self.regression1(logits)
        logits = self.fc3(features)
        regression3 = self.regression2(logits)
        logits = self.fc4(features)
        regression4 = self.regression2(logits)
        logits = self.fc5(features)
        regression5 = self.regression3(logits)
        logits = self.fc6(features)
        regression6 = self.regression3(logits)
        logits = self.fc7(features)
        regression7 = self.regression4(logits)
        logits = self.fc8(features)
        regression8 = self.regression4(logits)
        logits = self.fc9(features)
        regression9 = self.regression4(logits)
        new_features = features.clone()
        new_features = torch.cat((new_features, regression1,regression2,regression3,regression4,regression5,regression6,regression7,regression8,regression9),dim=1)
        logits = self.fc10(new_features)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('task3_moreF9_2.pkl'))
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task3_moreF9_2'+'.pkl')
        ...
        draw_chart(chart_data,'task3_moreF9_2')