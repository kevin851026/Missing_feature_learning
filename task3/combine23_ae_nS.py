...
class Model(nn.Module):
    ...
    def forward(self, features):
        ...
        new_feature = self.fc3(logits)
        regression = self.fc4(new_feature)
        regression = self.regression(regression)
        logits = self.fc5(new_feature)
...
if __name__ == '__main__':
    ...
    model.load_state_dict(torch.load('task3_combine23_ae_nS.pkl'))
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task3_combine23_ae_nS'+'.pkl')
        ...
        draw_chart(chart_data,'task3_combine23_ae_nS')