...
class Model(nn.Module):
    ...
    def forward(self, features):
        ...
        newfeature = self.fc3(logits)
        regression = self.regression(newfeature)
        logits = self.fc4(torch.cat((features,newfeature),dim=1))
...
if __name__ == '__main__':
    ...
    # model.load_state_dict(torch.load('task3_combine23_ae_2.pkl'))
    ...
    for epoch in range(15000):
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task3_combine23_ae_2'+'.pkl')
        ...
        draw_chart(chart_data,'task3_combine23_ae_2')