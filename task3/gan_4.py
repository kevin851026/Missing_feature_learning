...
if __name__ == '__main__':
    ...
    for epoch in range(15000):
        ...
        for d_frequence in range(1):
            ...
            for step, (batch) in enumerate(train_loader):
                ...
                optimizer_d.step()
        torch.save(model.D.state_dict(), 'task3_ganD_4'+'.pkl')
        #---- Train G to fool D-------
        for g_frequence in range(1):
            for step, (batch) in enumerate(train_loader):
                ...
                g_loss.backward()
                optimizer_g.step()
        torch.save(model.G.state_dict(), 'task3_ganG_4'+'.pkl')
    ...
    #           '\tG_loss: '+str(round(generator_loss/(step+1),5)))
        ...
        if v_acc > max_acc :
            max_acc = v_acc
            torch.save(model.state_dict(), 'task3_gan_4'+'.pkl')
        print('Epoch: '+str(epoch)+\
        ...
        draw_chart(chart_data,'task3_gan_4')